...
from quad_controller_rl.agents.actor_c import ActorC
from quad_controller_rl.agents.critic_c import CriticC
...
class DDPG_C(BaseAgent):
    ...
    def __init__(self, task):
        ...
        if self.load_weights:
            ...
            self.actor_filename = os.path.join(util.get_param('out'),
                "model_combine_{}_actor{}".format(util.get_timestamp(), self.model_ext))
            self.critic_filename = os.path.join(util.get_param('out'),
                "model_combine_{}_critic{}".format(util.get_timestamp(), self.model_ext))
            print("Actor filename :", self.actor_filename)  # [debug]
            print("Critic filename:", self.critic_filename)  # [debug]
        ...
        self.actor_local = ActorC(self.state_size, self.action_size, self.action_low, self.action_high, 0.001)
        self.actor_target = ActorC(self.state_size, self.action_size, self.action_low, self.action_high, 0.001)

        self.critic_local = CriticC(self.state_size, self.action_size, 0.001)
        self.critic_target = CriticC(self.state_size, self.action_size, 0.001)
        ...
        self.stats_filename = os.path.join(
            util.get_param('out'),
            "stats_combine_{}.csv".format(util.get_timestamp()))  # path to CSV file
    ...
    def preprocess_state(self, state):
        ...
        new_state = np.array([state[2],state[5],state[6]])
        return new_state  # position only
